    ...
    for j in range(N): # 테스트케이스 길이만큼 반복
        ...
        for k in range(5): # 각 빌딩마다 양옆 두 칸의 빌딩과 높이를 비교하는 반복문
            if k == 2:     # 자기 자신과의 비교하게 될 경우
                ...
                if max < buildings[j + k]: # 비교를 위해 양옆 두 칸의 빌딩의 크기 중 제일 큰 빌딩을 찾아서
                    max = buildings[j + k] # max 변수에 할당
        
        if max < case[j]:           # j번째 빌딩이 max보다 작다면
            ans += (case[j] - max)  # ans 변수에 층수의 차이만큼을 가산

    print(f'#{i + 1} {ans}') # 더해진 답안을 출력