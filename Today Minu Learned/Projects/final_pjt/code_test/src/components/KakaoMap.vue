...
export default {
  ...
  mounted() {
    if (!window.kakao || !window.kakao.maps) {
      ...
      kakaoMapScript.src = `//dapi.kakao.com/v2/maps/sdk.js?autoload=false&libraries=services,clusterer&appkey=${process.env.VUE_APP_KAKAO_API_KEY}`
      ...
      kakaoMapScript.addEventListener("load", () => {
        kakao.maps.load(() => {
          ...
          // 사용자 좌표 설정
          const latitude = this.userLocationData.latitude
          const longitude = this.userLocationData.longitude
          ...
          this.map = new kakao.maps.Map(kakaoMap, mapOption)
