def comb(n, depth):
    ...
    if depth == n:
        ...
        count = 0
        line_visit = []
        for jf in range(len(sel)):
            if core_line[jf + 1][sel[jf]][0] is True:
                if core_line[jf + 1][sel[jf]] == [False]
                for kf in line_visit:
                    if kf in core_line[jf + 1][sel[jf]][1]:
                        count = 999999999
                        break
                line_visit.extend(core_line[jf + 1][sel[jf]][1])
                count += len(core_line[jf + 1][sel[jf]][1])
